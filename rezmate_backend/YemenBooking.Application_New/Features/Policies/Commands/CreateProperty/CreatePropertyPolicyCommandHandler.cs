...
namespace YemenBooking.Application.Features.Policies.Commands.CreateProperty
{
    ...
    public class CreatePropertyPolicyCommandHandler : IRequestHandler<CreatePropertyPolicyCommand, ResultDto<Guid>>
    {
        ...
        public async Task<ResultDto<Guid>> Handle(CreatePropertyPolicyCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!HasAnyRules(request))
                return ResultDto<Guid>.Failed("يجب إدخال قواعد السياسة (عبر الحقول) أو إرسال JSON (Rules) للتوافق");
        ...
        private static bool HasAnyRules(CreatePropertyPolicyCommand request)
        {
            if (!string.IsNullOrWhiteSpace(request.Rules)) return true;

            return request.CancellationFreeCancel.HasValue ||
                   request.CancellationFullRefund.HasValue ||
                   request.CancellationRefundPercentage.HasValue ||
                   request.CancellationDaysBeforeCheckIn.HasValue ||
                   request.CancellationHoursBeforeCheckIn.HasValue ||
                   request.CancellationNonRefundable.HasValue ||
                   !string.IsNullOrWhiteSpace(request.CancellationPenaltyAfterDeadline) ||
                   request.PaymentDepositRequired.HasValue ||
                   request.PaymentFullPaymentRequired.HasValue ||
                   request.PaymentDepositPercentage.HasValue ||
                   request.PaymentAcceptCash.HasValue ||
                   request.PaymentAcceptCard.HasValue ||
                   request.PaymentPayAtProperty.HasValue ||
                   request.PaymentCashPreferred.HasValue ||
                   (request.PaymentAcceptedMethods != null && request.PaymentAcceptedMethods.Length > 0) ||
                   request.CheckInTime.HasValue ||
                   request.CheckOutTime.HasValue ||
                   request.CheckInFrom.HasValue ||
                   request.CheckInUntil.HasValue ||
                   request.CheckInFlexible.HasValue ||
                   request.CheckInFlexibleCheckIn.HasValue ||
                   request.CheckInRequiresCoordination.HasValue ||
                   request.CheckInContactOwner.HasValue ||
                   !string.IsNullOrWhiteSpace(request.CheckInEarlyCheckInNote) ||
                   !string.IsNullOrWhiteSpace(request.CheckInLateCheckOutNote) ||
                   !string.IsNullOrWhiteSpace(request.CheckInLateCheckOutFee) ||
                   request.ChildrenAllowed.HasValue ||
                   request.ChildrenFreeUnderAge.HasValue ||
                   request.ChildrenHalfPriceUnderAge.HasValue ||
                   request.ChildrenMaxChildrenPerRoom.HasValue ||
                   request.ChildrenMaxChildren.HasValue ||
                   !string.IsNullOrWhiteSpace(request.ChildrenCribsNote) ||
                   request.ChildrenPlaygroundAvailable.HasValue ||
                   request.ChildrenKidsMenuAvailable.HasValue ||
                   request.PetsAllowed.HasValue ||
                   !string.IsNullOrWhiteSpace(request.PetsReason) ||
                   request.PetsFeeAmount.HasValue ||
                   !string.IsNullOrWhiteSpace(request.PetsMaxWeight) ||
                   request.PetsRequiresApproval.HasValue ||
                   request.PetsNoFees.HasValue ||
                   request.PetsPetFriendly.HasValue ||
                   request.PetsOutdoorSpace.HasValue ||
                   request.PetsStrict.HasValue ||
                   request.ModificationAllowed.HasValue ||
                   request.ModificationFreeModificationHours.HasValue ||
                   !string.IsNullOrWhiteSpace(request.ModificationFeesAfter) ||
                   request.ModificationFlexible.HasValue ||
                   !string.IsNullOrWhiteSpace(request.ModificationReason);
        }
    }
}