...
import '../../../../core/utils/dynamic_field_serializer.dart';
...
class SearchRemoteDataSourceImpl implements SearchRemoteDataSource {
  ...
  Future<ResultDto<SearchPropertiesResponseModel>> searchProperties({
    ...
    try {
      ...
      if (dynamicFieldFilters != null && dynamicFieldFilters.isNotEmpty) {
        debugPrint(
            '🔧 [DataSource] dynamicFieldFilters BEFORE serialize: $dynamicFieldFilters');
        final serialized =
            DynamicFieldSerializer.serialize(dynamicFieldFilters);
        debugPrint('🔧 [DataSource] serialized: $serialized');
        serialized.forEach((key, value) {
          final paramKey = 'dynamicFieldFilters[$key]';
          queryParams[paramKey] = value;
          debugPrint('🔧 [DataSource] adding: $paramKey = $value');
        });
      }
