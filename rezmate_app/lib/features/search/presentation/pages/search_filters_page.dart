...
class _SearchFiltersPageState extends State<SearchFiltersPage>
  ...
  bool _validateFilters() {
    ...
    if (_dynamicFields.isNotEmpty) {
      final dynamicValues =
          _filters['dynamicFieldFilters'] as Map<String, dynamic>? ?? {};
  ...
  void _loadDynamicFieldsForUnitType(String unitTypeId) async {
    try {
      ...
      try {
        final cached =
            await _dataSyncService.getFilterableFieldsByUnitType(unitTypeId);
        setState(() {
  ...
  Widget _buildAmenitiesSection(SearchFilters? filters) {
    ...
    final amenities = selectedPropertyTypeId == null
        ...
        : allAmenities.where((a) {
            // بدون نوع عقار محدد نُظهر كل المرافق
            if (selectedPropertyTypeId == null) {
              return true;
            }

            // إذا لم يكن للمرفق أي propertyTypeIds فهذا يعني عدم وجود ربط صريح
            // نخفي هذا المرفق عند اختيار نوع عقار
            if (a.propertyTypeIds.isEmpty) {
              return false;
            }

            // نعرض المرفق فقط إذا كان مرتبطًا بنوع العقار المحدد
  ...
  Widget _buildDynamicUnitFieldsSection(SearchFilters? filters) {
    ...
    final dynamicFields = _dynamicFields.isNotEmpty
        ? _dynamicFields
        : (filters?.dynamicFieldValues ?? []);
    if (dynamicFields.isEmpty) return const SizedBox.shrink();

    final filterableFields = dynamicFields
        .where((field) => field.isSearchable == true && field.isPublic == true)
        .toList();
    ...
    final hasRequiredFields =
        unitFields.any((field) => field['isRequired'] == true);
    final hasError = hasRequiredFields &&
        unitFields.any((field) =>
            field['isRequired'] == true &&
            _requiredFieldsStatus[
                    'dynamicField_${field['fieldId'] ?? field['fieldName']}'] ==
                false);
