    ...
    try {
      final response = await apiClient.get(
        ...
        queryParameters: <String, dynamic>{
          ...
          if (adults != null) 'adults': adults,
          if (children != null) 'children': children,
          if (unitTypeId != null && unitTypeId.isNotEmpty)
      ...
      if (response.statusCode == 200) {
        ...
        // Debug: print first unit's field data to check API response
        if (unitsJson.isNotEmpty) {
          print('=== DEBUG: First Unit Data ===');
          final firstUnit = unitsJson.first;
          print('Unit ID: ${firstUnit['id']}');
          print('Unit Name: ${firstUnit['name']}');
          print('fieldValues count: ${(firstUnit['fieldValues'] as List?)?.length ?? 0}');
          print('dynamicFields count: ${(firstUnit['dynamicFields'] as List?)?.length ?? 0}');
          
          if (firstUnit['fieldValues'] is List && (firstUnit['fieldValues'] as List).isNotEmpty) {
            print('First fieldValue sample:');
            print('  ${(firstUnit['fieldValues'] as List).first}');
          }
          
          if (firstUnit['dynamicFields'] is List && (firstUnit['dynamicFields'] as List).isNotEmpty) {
            print('First dynamicField sample:');
            print('  ${(firstUnit['dynamicFields'] as List).first}');
          }
          print('=== END DEBUG ===');
        }
