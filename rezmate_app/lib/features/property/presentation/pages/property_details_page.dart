...
class _PropertyDetailsPageState extends State<PropertyDetailsPage>
  ...
  void initState() {
    ...
    // إذا كان هناك unitId محدد، سنقوم بتحديد الوحدة بعد تحميل البيانات
              ...
              child: Row(
                children: [
                  if (isLoadingPrice) ...[
                    ...
                    const SizedBox(width: 12),
                  ] else if (availability.hasAvailableUnits == false) ...[
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        mainAxisSize: MainAxisSize.min,
  ...
  /// بناء المحتوى المتسلسل بدلاً من التبويبات
  ...
  /// 5. التقييمات
  /// 6. الموقع
  Widget _buildSequentialContent(PropertyState state) {
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSection(
            ...
            child: PropertyInfoWidget(property: detailsState.property),
          ),

          const SizedBox(height: 16),

          // 2. المرافق والخدمات (تم حذف قسم الوحدات)
          if (detailsState.property.amenities.isNotEmpty) ...[
            _buildSection(
              title: 'المرافق والخدمات',
              icon: Icons.star_border,
              child: AmenitiesGridWidget(
                  amenities: detailsState.property.amenities),
            ),
            const SizedBox(height: 16),
          ],

          // 3. الخدمات المتاحة
          if (detailsState.property.services.isNotEmpty) ...[
            _buildServicesGrid(detailsState),
            const SizedBox(height: 16),
          ],

          // 5. السياسات
          if (detailsState.property.policies.isNotEmpty) ...[
            _buildSection(
              title: 'السياسات والقوانين',
              icon: Icons.policy,
              child: PoliciesWidget(policies: detailsState.property.policies),
            ),
            const SizedBox(height: 16),
          ],
          ...
          _buildSection(
            ...
            child: ReviewsSummaryWidget(
              propertyId: detailsState.property.id,
              reviewsCount: detailsState.property.reviewsCount,
              averageRating: detailsState.property.averageRating,
              onViewAll: () => _navigateToReviews(context, detailsState),
            ),
          ),
          ...
          _buildSection(
            ...
            child: LocationMapWidget(
              latitude: detailsState.property.latitude,
              longitude: detailsState.property.longitude,
              propertyName: detailsState.property.name,
              address: detailsState.property.address,
            ),
          ),

          // مساحة إضافية في الأسفل للـ bottom bar
...
// تحديث دالة بناء الخدمات في Flutter
  ...
  Widget _buildServicesGrid(PropertyDetailsLoaded state) {
    final services = state.property.services;
        ...
        child: Column(
          ...
          children: [
            ...
            ...List.generate(services.length, (index) {
              ...
              final color = _getServiceColorFromName(service.name);
              final isLast = index == services.length - 1;
              return _buildServiceTimelineRow(service, color, isLast);
            }),
          ],
        ),
      ),
  ...
  Widget _buildServiceTimelineRow(
      PropertyService service, Color color, bool isLast) {
    return Padding(
...
// دالة لتحويل اسم الأيقونة من السلسلة النصية إلى IconData
  ...
  void _openGallery(
    ...
    if (state.property.units.isEmpty) {
      // إنشاء وحدة وهمية من صور العقار
      final propertyImages = state.property.images.map((img) {
        return UnitImage(
          id: img.id,
          url: img.url,
          caption: img.caption,
      ...
      if (propertyImages.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('لا توجد صور لعرضها'),
      ...
      unitsToDisplay = [
        Unit(
          ...
          unitTypeName: state.property.typeName,
      ...
      unitsToDisplay = state.property.units;
    }

    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => PropertyGalleryPage(
  ...
  Widget _buildBookNowButton(BuildContext context, PropertyState state,
      String selectedUnitId, List<dynamic> units) {
    return _buildGlowingButton(
      onPressed: () {
        // بدلاً من فتح صفحة نموذج الحجز مباشرة، ننتقل إلى صفحة الوحدات التفصيلية
        final property = state is PropertyWithDetails
            ? state.property
            : (state as PropertyDetailsLoaded).property;

        // جلب التواريخ والضيوف من FilterStorageService (اختيارات البحث/الهوم)
        final selections = sl<FilterStorageService>().getHomeSelections();
        final DateTime? checkIn = selections['checkIn'] as DateTime?;
        final DateTime? checkOut = selections['checkOut'] as DateTime?;
        ...
        final String? unitTypeId = selections['unitTypeId'] as String?;
        final String currency = sl<LocalStorageService>().getSelectedCurrency();
