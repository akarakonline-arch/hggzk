...
class FilterStorageService {
  ...
  Future<void> saveHomeSelections({
    ...
    final serializedFilters = _serializeMap(dynamicFilters);
    await _prefs.setString(_kDynamicFieldFilters, jsonEncode(serializedFilters));
  }
  ...
  Map<String, dynamic> getHomeSelections() {
    ...
    if (dynamicFiltersJson != null) {
      try {
        final decoded = Map<String, dynamic>.from(jsonDecode(dynamicFiltersJson));
        dynamicFilters = _deserializeMap(decoded);
      } catch (_) {}
    }
  ...
  Future<void> saveCurrentFilters(Map<String, dynamic> filters) async {
    ...
    if (f['dynamicFieldFilters'] is Map<String, dynamic>) {
      f['dynamicFieldFilters'] = _serializeMap(f['dynamicFieldFilters']);
    }
  ...
  Map<String, dynamic>? getCurrentFilters() {
    ...
    try {
      ...
      if (m['dynamicFieldFilters'] is Map<String, dynamic>) {
        m['dynamicFieldFilters'] = _deserializeMap(m['dynamicFieldFilters']);
      }
      return m;
    } catch (_) {
      return null;
