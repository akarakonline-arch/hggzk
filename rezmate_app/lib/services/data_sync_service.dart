import 'package:rezmate/services/local_data_service.dart';
import 'package:rezmate/services/connectivity_service.dart';
import 'package:rezmate/features/home/data/models/property_type_model.dart';
import 'package:rezmate/features/home/data/models/unit_type_model.dart';
import 'package:rezmate/features/search/data/models/search_filter_model.dart';
import 'package:rezmate/features/reference/data/datasources/reference_remote_datasource.dart';
import 'package:rezmate/features/reference/data/models/city_model.dart';
import 'package:rezmate/features/reference/data/models/currency_model.dart';
import 'package:rezmate/features/home/data/datasources/home_remote_datasource.dart';
    ...
    try {
      ...
      if (isConnected) {
        ...
        if (remoteData.isNotEmpty) {
          // دمج الأنواع الجديدة مع الأنواع المخزنة بدلاً من استبدالها بالكامل
          final existing = _localDataService.getUnitTypes();
          final filteredExisting = existing
              .where((u) => u.propertyTypeId != propertyTypeId)
              .toList();
          final merged = <UnitTypeModel>[...filteredExisting, ...remoteData];

          await _localDataService.saveUnitTypes(merged);

          return remoteData;
        } else {
          // إذا كان رد الباك اند فارغاً، حاول استخدام البيانات المخزنة لنفس نوع العقار
          final localData =
              _localDataService.getUnitTypesByPropertyType(propertyTypeId);
          if (localData.isNotEmpty) {
            return localData;
          }
