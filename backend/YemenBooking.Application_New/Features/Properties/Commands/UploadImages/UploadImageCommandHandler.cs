    ...
    public class UploadImageCommandHandler : IRequestHandler<UploadImageCommand, ResultDto<ImageDto>>
    {
        ...
        public async Task<ResultDto<ImageDto>> Handle(UploadImageCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!isVideo)
            {
                ...
                // توليد معرف فريد لجميع الملفات (الأصلي والمصغرات)
                var uniqueId = Guid.NewGuid().ToString("N").Substring(0, 8);
                var fileName = $"{request.Name}_{uniqueId}{request.Extension}";
                ...
                if (!isVideo && request.GenerateThumbnail)
                {
                    ...
                    {
                        var thumbName = $"{request.Name}_{uniqueId}_thumb{request.Extension}";
                    ...
                    {
                        var thumbName = $"{request.Name}_{uniqueId}_thumb48{request.Extension}";
                        await _fileStorageService.UploadFileAsync(
                    ...
                    {
                        var thumbName = $"{request.Name}_{uniqueId}_thumb64{request.Extension}";
                        await _fileStorageService.UploadFileAsync(
