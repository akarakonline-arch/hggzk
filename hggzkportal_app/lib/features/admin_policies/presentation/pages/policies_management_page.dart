...
class _PoliciesManagementPageState extends State<PoliciesManagementPage>
  ...
  // أنواع السياسات المستخدمة في التبويبات (بدون سياسات الحيوانات الأليفة)
  final List<PolicyType> _tabPolicyTypes = const [
  ...
  void initState() {
    ...
    // تبويب "الكل" + 4 أنواع (إلغاء، دخول، أطفال، دفع) بدون تبويب الحيوانات
      ...
      child: Container(
        ...
        child: TabBar(
          ...
          onTap: (index) {
            ...
            if (index > 0 && index - 1 < _tabPolicyTypes.length) {
              selectedType = _tabPolicyTypes[index - 1];
            }
            context.read<PoliciesBloc>().add(
                  ApplyFiltersEvent(
                    propertyId:
