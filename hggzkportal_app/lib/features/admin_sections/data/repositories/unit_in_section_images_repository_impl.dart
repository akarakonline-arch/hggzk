// lib/features/admin_sections/data/repositories/unit_in_section_images_repository_impl.dart
...
class UnitInSectionImagesRepositoryImpl
    implements UnitInSectionImagesRepository {
    ...
    if (await networkInfo.isConnected) {
      try {
        final List<SectionImageModel> result = await remoteDataSource
            .getUnitInSectionImages(unitInSectionId, tempKey: tempKey);
        return Right(result);
      } on ServerException catch (e) {
        return Left(ServerFailure(e.message));
      } catch (e) {
    ...
    if (await networkInfo.isConnected) {
      try {
        final bool result = await remoteDataSource.reorderImages(
            unitInSectionId, tempKey, imageIds);
        return Right(result);
      } on ServerException catch (e) {
        return Left(ServerFailure(e.message));
      } catch (e) {
    ...
    if (await networkInfo.isConnected) {
      try {
        final bool result = await remoteDataSource.setAsPrimaryImage(
            unitInSectionId, tempKey, imageId);
        return Right(result);
      } on ServerException catch (e) {
        return Left(ServerFailure(e.message));
      } catch (e) {
