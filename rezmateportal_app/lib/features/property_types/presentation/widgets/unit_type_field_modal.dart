...
class _UnitTypeFieldModalState extends State<UnitTypeFieldModal>
  ...
  /// تحويل الخيارات إلى نص (تدعم List و Map و أي نوع آخر)
  String _parseOptionsToString(dynamic options) {
    if (options == null) return '';

    // Debug: طباعة نوع البيانات الفعلي
    debugPrint(
        '🔍 _parseOptionsToString - type: ${options.runtimeType}, value: $options');
    ...
    // إذا كانت خريطة
    if (options is Map) {
      // تحقق من وجود مفتاح 'options' داخل الخريطة (حالة خاصة)
      ...
      // تحقق مما إذا كانت الخريطة عبارة عن قائمة مُحولة (مفاتيح رقمية)
      final keys = options.keys.toList();
      final isNumericKeys =
          keys.every((k) => int.tryParse(k.toString()) != null);

      if (isNumericKeys) {
        // خريطة بمفاتيح رقمية = قائمة مُحولة
      ...
      // إذا كانت القيم قوائم، استخدم القائمة الأولى
