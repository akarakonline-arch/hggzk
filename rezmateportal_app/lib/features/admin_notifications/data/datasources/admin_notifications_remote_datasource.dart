...
class AdminNotificationsRemoteDataSource {
  ...
  Future<int> broadcast({
    ...
    final res =
        await apiClient.post('/api/admin/notifications/broadcast', data: {
      'type': type,
      'title': title,
      'message': message,
  ...
  Future<bool> resend(String notificationId) async {
    final res =
        await apiClient.post('/api/admin/notifications/$notificationId/resend');
    return (res.data['data'] ?? res.data['result']) as bool? ?? true;
  }
  ...
  Future<PaginatedResult<AdminNotificationEntity>> getSystem({
    ...
    final res =
        await apiClient.get('/api/admin/notifications', queryParameters: {
      'pageNumber': page,
      'pageSize': pageSize,
  ...
  Future<PaginatedResult<AdminNotificationEntity>> getUser({
    ...
    final res = await apiClient
        .get('/api/admin/notifications/user/$userId', queryParameters: {
      'pageNumber': page,
      'pageSize': pageSize,
