...
class PropertyImageGallery extends StatefulWidget {
  ...
  final List<PropertyImage>? initialImages;
...
class PropertyImageGalleryState extends State<PropertyImageGallery>
  ...
  PropertyImagesBloc? _imagesBloc;

  // Local state management
  List<String> _localImages = [];
  List<PropertyImage> _remoteImages = [];
  List<PropertyImage> _displayImages = [];
    ...
    if (!_isLocalMode && !_isInitialLoadDone) {
      try {
        ...
        if (_imagesBloc != null &&
          ...
          _imagesBloc!.add(LoadPropertyImagesEvent(
              propertyId: widget.propertyId, tempKey: widget.tempKey));
          _isInitialLoadDone = true;
        }
      } catch (e) {
        debugPrint('PropertyImagesBloc not available, working in local mode');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<PropertyImagesBloc, PropertyImagesState>(
      bloc: _imagesBloc,
  ...
  Future<void> handleMediaFile(String filePath, {required bool isVideo}) async {
    if (_isLocalMode) {
      ...
      setState(() {
        ...
        // تابع property_image_gallery.dart من حيث توقفنا
      });
      if (widget.onLocalImagesChanged != null) {
        widget.onLocalImagesChanged!(_localImages);
      }
        ...
        if (imagesToAdd.isNotEmpty) {
          if (_isLocalMode) {
            ...
            showSuccessSnackBar(
              ...
              (widget.propertyId != null || widget.tempKey != null)) {
            setState(() {
  ...
  void toggleSelectionMode() {
    setState(() {
      ...
      if (!_isSelectionMode) {
        ...
        _selectedLocalIndices.clear();
        if (_imagesBloc != null) {
          _imagesBloc!.add(const DeselectAllImagesEvent());
        }
  ...
  Widget buildReorderableGrid(List<PropertyImage> images) {
    return ReorderableWrap(
      spacing: 8,
      runSpacing: 8,
      needsLongPressDraggable: false,
  ...
  Widget buildReorderableImageItem(PropertyImage image, int index) {
  ...
  void reorderImages(int oldIndex, int newIndex) {
    ...
    if (_imagesBloc != null &&
        (widget.propertyId != null || widget.tempKey != null)) {
      _imagesBloc!.add(ReorderImagesEvent(
        propertyId: widget.propertyId,
        tempKey: widget.tempKey,
  ...
  void showMediaOptionsMenu(PropertyImage media, int index) {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
  ...
  void setPrimaryImage(int index) {
    ...
    if (_imagesBloc != null &&
      ...
      _imagesBloc!.add(SetPrimaryImageEvent(
        propertyId: widget.propertyId,
        tempKey: widget.tempKey,
  ...
  void confirmDeleteImage(PropertyImage media) {
    showDialog(
      fullscreenDialog: true,
      context: context,
      builder: (context) => _DeleteConfirmationDialog(
        ...
        onConfirm: () {
          ...
          if (_imagesBloc != null) {
            HapticFeedback.lightImpact();
            _imagesBloc!.add(DeletePropertyImageEvent(imageId: media.id));
          }
    ...
    showDialog(
      ...
      builder: (context) => _DeleteConfirmationDialog(
        ...
        onConfirm: () {
          ...
          if (_imagesBloc != null) {
            _imagesBloc!.add(DeleteMultipleImagesEvent(
  ...
  Widget buildOptimizedImage(PropertyImage image) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final dpr = MediaQuery.of(context).devicePixelRatio;
        final targetWidth = (constraints.maxWidth.isFinite
  ...
  void handleStateChanges(PropertyImagesState state) {
    ...
    } else if (state is PropertyImageUploaded) {
      setState(() {
    ...
    } else if (state is PropertyImagesError) {
      setState(() {
        _uploadingFiles.clear();
        _uploadingFileObjects.clear();
        _uploadProgress.clear();
    ...
    } else if (state is PropertyImageUploading) {
  ...
  List<PropertyImage> getImagesFromState(PropertyImagesState state) {
    if (state is PropertyImagesLoaded) return state.images;
    if (state is PropertyImageUploaded) return state.allImages;
    if (state is MultipleImagesUploaded) return state.allImages;
    if (state is PropertyImageDeleted) return state.remainingImages;
    ...
    if (state is PropertyImageUpdated) return state.updatedImages;
    if (state is PropertyImageUploading) return state.currentImages;
    if (state is PropertyImageDeleting) return state.currentImages;
    if (state is PropertyImageUpdating) return state.currentImages;
    if (state is ImagesReordering) return state.currentImages;
    if (state is PropertyImagesError) return state.previousImages ?? [];
  ...
  Widget buildEnhancedHeader(int totalCount, PropertyImagesState state) {
    final isSelectionMode = state is PropertyImagesLoaded
        ? state.isSelectionMode
        : _isSelectionMode;
    final selectedCount = state is PropertyImagesLoaded
  ...
  Widget buildMainContent(
      List<PropertyImage> images, PropertyImagesState state) {
    if (state is PropertyImagesLoading && images.isEmpty) {
  ...
  Widget buildEnhancedImagesGrid(
      List<PropertyImage> images, PropertyImagesState state) {
    final selectedImageIds = state is PropertyImagesLoaded
      ...
      child: GestureDetector(
        onTap: () {
          if (isSelectionMode) {
            HapticFeedback.lightImpact();
            if (_imagesBloc != null) {
              _imagesBloc!.add(ToggleImageSelectionEvent(imageId: media.id));
            } else {
              setState(() {
  ...
  Widget buildRemoteVideoThumbnail(PropertyImage video) {
  ...
  void previewVideo(PropertyImage video) {
  ...
  void previewImage(PropertyImage image) {
    Navigator.of(context).push(
      PageRouteBuilder(
        opaque: false,
        pageBuilder: (context, animation, secondaryAnimation) {
